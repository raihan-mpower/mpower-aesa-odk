...
import org.odk.collect.android.views.ODKView2;
...
OnGestureListener {
	...
	public boolean start_repeat_list = false;
	public int repeat_question_index = 0;
	...
	public void onCreate(Bundle savedInstanceState) {
		...
		repeatlayout = new LinearLayout(this);
	...
	private boolean saveAnswersForCurrentScreen(boolean evaluateConstraints) {
		...
		if (formController.currentPromptIsQuestion()) {
			...
			if(start_repeat_list){
				answers = ((ODKView2) (((LinearLayout)mCurrentView)).getChildAt(repeat_question_index-1))
				.getAnswers();

			}else{
				answers = ((ODKView) mCurrentView)
				.getAnswers();
			}
	...
	private View createView(int event, boolean advancingPage) {
		...
		Log.v("alphachan omega", "form index"+formController.getFormIndex());
		switch (event) {
		case FormEntryController.EVENT_BEGINNING_OF_FORM:
			Log.v("alphachan omega", "2");
			View startView = View
		...
		case FormEntryController.EVENT_REPEAT:
			...
			if(!start_repeat_list){
				...
				try {
					...
					FormEntryCaption[] groups = formController
					.getGroupsForCurrentIndex();
					odkv = new ODKView2(this, formController.getQuestionPrompts(),
							groups, advancingPage);
					Log.i(t,
							"created view for group "
							+ (groups.length > 0 ? groups[groups.length - 1]
				...
				if(repeat_question_index == 0){
					if(!(repeatlayout.getChildCount()>0)){
					repeatlayout.addView(odkv);
					}
					repeat_question_index++;
					return repeatlayout;
				}else{
					repeatlayout.addView(odkv);
					repeat_question_index++;
	...
	private void showNextView() {
		...
		case FormEntryController.EVENT_END_OF_FORM:
		case FormEntryController.EVENT_REPEAT:
			Log.v("alphachan omega", "7");
			next = createView(event, true);
			showView(next, AnimationType.RIGHT);
			break;
		case FormEntryController.EVENT_PROMPT_NEW_REPEAT:
			Log.v("alphachan omega", "8");
			start_repeat_list = true;
			createRepeatDialog();
			break;
		case FormEntryController.EVENT_REPEAT_JUNCTURE:
			Log.v("alphachan omega", "9");
			start_repeat_list = true;
			...
			public void onClick(DialogInterface dialog, int i) {
				...
				case DialogInterface.BUTTON_POSITIVE: // yes, repeat
					...
					try {
						...
						Log.v("alphachan omega", "is this called more than once");
				...
				case DialogInterface. BUTTON_NEGATIVE: // no, no repeat
					...
					.logInstanceAction(this, "createRepeatDialog",
					"showNext");
					repeat_question_index = 0;
					repeatlayout.removeAllViews();
					mQuestionHolder.removeView(repeatlayout);
					showNextView();
					break;
				}
			}
